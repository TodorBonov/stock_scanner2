"""
New pipeline (4/5) 6MO: ChatGPT analysis for existing positions using 6-month OHLCV only.
Reads prepared_existing_positions_6mo.json, same prompts and 8000 cap as New4. Run New3 then New3_6mo first.
"""
import os
import json
import time
...
def main():
    parser = argparse.ArgumentParser(description="New4 6MO: ChatGPT existing positions (6 months OHLCV). Run New3 then New3_6mo first.")
    ...
    if not PREPARED_EXISTING.exists():
        print(f"[ERROR] Prepared data not found: {PREPARED_EXISTING}. Run New3 then New3_prepare_chatgpt_data_6mo.py first.")
        return
    ...
    if not positions:
        print("No positions in prepared data (run New2 to fetch positions, then New3, then New3_6mo).")
        return
    ...
    print(f"\n{'='*80}")
    print("NEW4 6MO: CHATGPT EXISTING POSITIONS (6 months OHLCV)")
    print(f"{'='*80}")
    print(f"Positions: {len(positions)}  Model: {model}")
    print(f"{'='*80}\n")

    report_lines = ["=" * 80, "NEW4 6MO: CHATGPT EXISTING POSITION SUGGESTIONS (6 months OHLCV)", "=" * 80, f"Generated: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}", f"Model: {model}", ""]
    total_tokens = 0
    ...
    out_file = NEW_PIPELINE_REPORTS / f"chatgpt_existing_positions_6mo_{ts}.txt"
