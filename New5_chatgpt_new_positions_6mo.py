"""
New pipeline (5/5) 6MO: ChatGPT analysis for new position candidates using 6-month OHLCV only.
Reads prepared_new_positions_6mo.json, same prompts and 8000 cap as New5, default limit 50. Run New3 then New3_6mo first.
"""
import os
import json
import time
...
def main():
    parser = argparse.ArgumentParser(description="New5 6MO: ChatGPT new position suggestions (6 months OHLCV). Run New3 then New3_6mo first.")
    ...
    if not PREPARED_NEW.exists():
        print(f"[ERROR] Prepared data not found: {PREPARED_NEW}. Run New3 then New3_prepare_chatgpt_data_6mo.py first.")
        return
    ...
    if not stocks:
        print("No A+/A stocks in prepared data (run New1 and New3, then New3_6mo; ensure scan produces A+ or A).")
        return
    ...
    print(f"\n{'='*80}")
    print("NEW5 6MO: CHATGPT NEW POSITION SUGGESTIONS (A+/A, 6 months OHLCV)")
    print(f"{'='*80}")
    print(f"Stocks: {len(stocks)}  Model: {model}")
    print(f"{'='*80}\n")

    report_lines = ["=" * 80, "NEW5 6MO: CHATGPT NEW POSITION SUGGESTIONS (A+/A, 6 months OHLCV)", "=" * 80, f"Generated: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}", f"Model: {model}", ""]
    ...
    out_file = NEW_PIPELINE_REPORTS / f"chatgpt_new_positions_6mo_{ts}.txt"
